...
inputs = [[(x/10)] for x in range(-30, 31)]
targets = [math.sin(x[0]) for x in inputs]

n = MLP(1, [10, 10, 1])  # 1 input, 2 hidden layers with 4 neurons each, and 1 output neuron

# Train the model using the new train function
loss = train(n, inputs, targets, steps=1000, learningRate=0.01)
...
draw_dot(loss).render('sine', view=True)  # This will create a file named 'quadratic.svg' and open it
